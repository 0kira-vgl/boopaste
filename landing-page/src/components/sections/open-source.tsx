...
export function OpenSource() {
  ...
  return (
    <section className="mx-auto flex max-w-2xl flex-col gap-6 px-6 py-24">
      ...
      <DistortText className="text-lg leading-relaxed text-foreground/90">{t.openSource.p1}</DistortText>
      <DistortText className="text-lg leading-relaxed text-foreground/70">{t.openSource.p2}</DistortText>
      <a
        href={SITE.githubUrl}
        target="_blank"
      ...
      >
        <GithubIcon size={16} />
        <DistortText>{t.openSource.cta}</DistortText>
      </a>
    </section>
  );
}
